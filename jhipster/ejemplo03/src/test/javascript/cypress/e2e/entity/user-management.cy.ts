...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'MarcoAntonio_AvilaRangel98', email: 'Marta16@yahoo.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Natalia.MunguiaTafoya');
      cy.get(`[data-cy="login"]`).should('have.value', 'Natalia.MunguiaTafoya');

      cy.get(`[data-cy="firstName"]`).type('Mayte');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Mayte');

      cy.get(`[data-cy="lastName"]`).type('Camacho Castellanos');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Camacho Castellanos');

      cy.get(`[data-cy="email"]`).type('Catalina_CortesAlejandro36@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Catalina_CortesAlejandro36@hotmail.com');
