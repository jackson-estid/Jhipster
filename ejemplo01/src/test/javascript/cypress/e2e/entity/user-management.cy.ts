...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Ramona.SaldivarDominguez', email: 'Enrique99@hotmail.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Caridad_SevillaSanabria');
      cy.get(`[data-cy="login"]`).should('have.value', 'Caridad_SevillaSanabria');

      cy.get(`[data-cy="firstName"]`).type('Jesús');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Jesús');

      cy.get(`[data-cy="lastName"]`).type('Olivas Marroquín');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Olivas Marroquín');

      cy.get(`[data-cy="email"]`).type('Ignacio67@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Ignacio67@gmail.com');
